...
\\　本年度は確認走行区間の達成であったが、今後選択課題に挑戦することを見据えて信号認識横断の課題に
本課題を選択した理由は、ロボットの機体がその場になくてもデータ収集と手法確認を行えるため、自律走行のデバック作業に影響を与えず独自に開発を進められる点が挙げられる。
本課題では、日の当たり方が大きく異なる信号をミスなくかつ素早く検出することが求められる。
そのためカメラはホワイトバランス調整などが行え、同時に安価であることが好ましい。
そこでカメラにはスマートフォンを採用した。
一般的なスマートフォンには上記の機能が備わっており、旧型モデルを用いることで価格も抑制できる。
本チームでは2020年発売のGalaxy A21を採用した。
\\　本課題では、信号検出の他に、横断歩道内で停止している自動車の有無などの安全確認が求められる。
