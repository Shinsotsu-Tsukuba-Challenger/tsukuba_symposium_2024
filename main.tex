...
\section{その他の試み}
本章では、つくばチャレンジの自律走行においては未だ試していない
本チームの取り組みについて紹介する。
\subsection{選択課題B　信号認識横断についての取り組み}
○信号認識の目的
...
そこでカメラには、スマートフォンの一種である、Galaxy A21を使用した。
一般的なスマートフォンには上記の機能が備わっており、発売が2020年であるため、比較的安価に入手可能である。
\\○課題点
\\信号横断を行うには、信号検出の他に、自動車が横断歩道内に停止しているかなどの安全確認が求められる。
そこで深層学習による画像認識を用いることでこれらの問題を対処する。
画像認識モデルには、軽量かつ交通認識に特化した、\cite{BabaVIDVIP}
\\○手法の概要
\\○結果
\\○残課題
